...
object Job {
   ...
   private def getStream(env: StreamExecutionEnvironment, dataPath: String): DataStream[Payload] = {
      ...
      header match {
         ...
         action = "Insert"
         relation = "lineitem"
         val i = Tuple6(cells(5).toDouble,cells(6).toDouble,format.parse(cells(10)),cells(4).toDouble,cells(3).toInt,cells(0).toLong)
         cnt = cnt + 1
         ...
         Array[String]("EXTENDEDPRICE","DISCOUNT","SHIPDATE","QUANTITY","LINENUMBER","ORDERKEY"), cnt))
         case "-LI" =>
         action = "Delete"
         relation = "lineitem"
         val i = Tuple6(cells(5).toDouble,cells(6).toDouble,format.parse(cells(10)),cells(4).toDouble,cells(3).toInt,cells(0).toLong)
