...
object Q6Job {
   ...
   private def getStream(env: StreamExecutionEnvironment, dataPath: String): DataStream[Payload] = {
      ...
      header match {
         ...
         action = "Insert"
         relation = "lineitem"
         val i = Tuple6(cells(0).toLong,format.parse(cells(10)),cells(4).toDouble,cells(3).toInt,cells(6).toDouble,cells(5).toDouble)
         cnt = cnt + 1
         ...
         Array[String]("ORDERKEY","SHIPDATE","QUANTITY","LINENUMBER","DISCOUNT","EXTENDEDPRICE"), cnt))
         case "-LI" =>
         action = "Delete"
         relation = "lineitem"
         val i = Tuple6(cells(0).toLong,format.parse(cells(10)),cells(4).toDouble,cells(3).toInt,cells(6).toDouble,cells(5).toDouble)
