...
import org.hkust.ProcessFunction.Q6.{Q6AggregateProcessFunction, Q6lineitemProcessFunction}
...
object Job {
   ...
   private def getStream(env: StreamExecutionEnvironment, dataPath: String): DataStream[Payload] = {
      ...
      val i = Tuple4(cells(6).toDouble,format.parse(cells(10)),cells(5).toDouble,cells(4).toDouble)
      var relation = ""
      var action = ""
      header match {
      case "+LI" =>
      ...
      Array("L_DISCOUNT","L_SHIPDATE","L_EXTENDEDPRICE","L_QUANTITY"), cnt)
