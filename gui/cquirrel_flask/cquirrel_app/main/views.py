...
def upload_json_file():
    ...
    cquirrel_app.send_codegen_log_data_to_client(codegen_log_result)

    # test if flink cluster is running
    if not cquirrel_utils.is_flink_cluster_running():
    ...
    t = threading.Thread(target=cquirrel_utils.run_flink_task, args=(config.GENERATED_JAR_FILE, query_idx))
    t.start()
